...
const QUERIES = {
  ...
  nodeCpu: '100 - (avg by (node) (rate(node_cpu_seconds_total{mode="idle"}[5m])) * 100)',
  // Node memory usage percentage
  nodeMemoryPercent: '(1 - node_memory_MemAvailable_bytes / node_memory_MemTotal_bytes) * 100',
  ...
  // Pod memory limit
  podMemoryLimit: 'sum(container_spec_memory_limit_bytes{container!=""}) by (pod, namespace)',
  // Cluster totals
  clusterCpu: 'sum(rate(node_cpu_seconds_total{mode!="idle"}[5m])) / sum(rate(node_cpu_seconds_total[5m])) * 100',
  clusterMemoryUsed: 'sum(node_memory_MemTotal_bytes - node_memory_MemAvailable_bytes)',
...
export function getPodMetrics(namespace: string, podName: string): ResourceMetrics | undefined {
...
export function getUsageGradient(percent: number): string {
  if (percent < 50) return 'linear-gradient(90deg, rgba(10, 255, 104, 0.3), rgba(10, 255, 104, 0.6))';
  if (percent < 80) return 'linear-gradient(90deg, rgba(252, 238, 10, 0.3), rgba(252, 238, 10, 0.6))';
