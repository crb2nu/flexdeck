      ...
      <PageScrollBody contentClass="gap-4">
        ...
        <Show when={metricsTab() === "grafana"}>
          <ErrorBoundary
            fallback={(err) => (
              <ErrorState
                message={`Failed to load Grafana dashboards: ${err.message}`}
                variant="banner"
              />
            )}
          >
...
const MetricCard: Component<{ panel: MetricPanel; loading: boolean }> = (
  props,
) => {
  ...
  const formatValue = (
    value: number,
    unit: string,
    precision: "headline" | "stat" | "axis" | "tooltip" = "headline",
  ): string => {
  ...
  const stats = createMemo(() => {
    ...
    if (values.length === 0)
      return { current: 0, min: 0, max: 0, avg: 0, trend: "stable" as const };
  ...
  return (
    <div class="surface-hover group flex flex-col gap-2 p-3 transition-colors hover:border-white/10">
      ...
      <div class="relative h-44 flex-1 sm:h-52">
        ...
        {/* Hover tooltip */}
        <Show
          when={hoveredIndex() !== null && props.panel.values[hoveredIndex()!]}
          keyed
        >
          {(val) => (
