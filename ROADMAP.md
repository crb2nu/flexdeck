...
- ◐ **Service-To-Cluster Binding (Partial)**: Inferred binding from inventory metadata, **upgraded to `verified`** by matching the repo's project path to a live Flux `GitRepository` source, then joined to live **K8s Deployments** (via the `kustomize.toolkit.fluxcd.io` labels) for the authoritative running namespace and aggregate replica health (`ready/desired`), shown on Stack cards (2026-06-08). The Deployment namespace overrides the inferred guess (e.g. `flexinfer` → `flexinfer-system`). Remaining: StatefulSet/DaemonSet + pod/rollout health, and image-label / Loom HUD signals.
